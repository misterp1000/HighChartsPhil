    ...
    setTimeout(() => {
      ...
      if (chart) {
        chartBatch.series.forEach(({ action, data }) => {
          ...
          else if (action === 'remove') chart.series.find(s => s.name === data)?.remove(false);
        });
        if (chartBatch.plotLines) chart.xAxis[0].update({ plotLines: chartBatch.plotLines }, false);
        chart.redraw();
      }
...
// Consolidated option state management
const toggleOption = (strike, isCall, isLong) => {
  const side = isCall ? 'calls' : 'puts';
  const existing = selectedOptions[side].findIndex(o => o.strike === strike && o.expiration === activeExpiration);
  ...
  if (existing >= 0) {
    ...
    const optionData = optionsChain[activeExpiration][side].find(o => o.strike === strike);
    if (!optionData) return null;
    selectedOptions[side].push({
      strike, premium: optionData.last, isLong, expiration: activeExpiration,
      rowIndex: optionsChain[activeExpiration][side].findIndex(o => o.strike === strike)
    });
    return 'add';
  }
};
...
document.addEventListener("click", function (e) {
  ...
  const strike = parseFloat(strikeCell.querySelector(".strike-value")?.textContent);
  if (isNaN(strike)) return;

  const isCall = btn.classList.contains("call-long") || btn.classList.contains("call-short");
  const isLong = btn.classList.contains("call-long") || btn.classList.contains("put-long");
  ...
  if (action !== 'remove') {
    ...
    if (isCall) {
      strikeCell.querySelector(isLong ? ".call-short" : ".call-long")?.classList.add("unselected");
    } else {
      strikeCell.querySelector(isLong ? ".put-short" : ".put-long")?.classList.add("unselected");
    }
  }

  // Update row highlighting
  const hasOptions = selectedOptions.calls.some(c => c.strike === strike && c.expiration === activeExpiration) ||
                   selectedOptions.puts.some(p => p.strike === strike && p.expiration === activeExpiration);
  btn.closest("tr").classList.toggle("row-highlight", hasOptions);

  // Batch chart updates
  batchChartUpdate(() => {
    const optionType = isCall ? "Call" : "Put";
    const seriesName = `${isLong ? "Long" : "Short"} ${optionType} ${strike} (${formatExpirationDate(activeExpiration)})`;
    
    if (action === 'remove') {
      chartBatch.series.push({ action: 'remove', data: seriesName });
    } else {
      if (action === 'update') {
        chartBatch.series.push({ action: 'remove', data: seriesName.replace(isLong ? "Long" : "Short", isLong ? "Short" : "Long") });
      }
...
function updateOptionsGrid(expirationDate) {
  ...
  if (chartComponent?.chart) {
    ...
    $$(".option-btn").forEach(btn => btn.classList.remove("selected", "unselected"));
    $$("tr").forEach(row => row.classList.remove("row-highlight"));
    
    const optionsContainer = $("#selected-options-container");
    if (optionsContainer) optionsContainer.innerHTML = "<p>No options selected</p>";
    ...
    if (strategyInfoText) strategyInfoText.textContent = "Select options to view strategy analysis";
  }
  ...
  try {
    ...
    // Update the connector's data to point to the new expiration's data
    board.dataPool.getConnector("optionsData").then(connector => {
      if (connector) {
        ...
        const newData = optionsChain[expirationDate].calls.map((call, index) => {
        ...
        // Try different ways to update the data property
        ...
        // Trigger a reload of the connector
        if (connector.load) {
          connector.load();
...
//Adds option payoff lines to the chart
function updateOptionInChart(strike, isCall, isLong) {
  ...
  const optionType = isCall ? "Call" : "Put";
  const seriesName = `${isLong ? "Long" : "Short"} ${optionType} ${strike} (${formatExpirationDate(activeExpiration)})`;
  const optionSeriesCount = chart.series.filter(s => s.name !== "Combined Strategy").length;

  // Check max options limit
  if (optionSeriesCount >= MAX_OPTIONS_IN_CHART && !chart.series.find(s => s.name === seriesName)) {
    showMaxOptionsWarning();
    return;
  }

  const rowData = optionsChain[activeExpiration][isCall ? "calls" : "puts"].find(o => o.strike === strike);
  if (!rowData) return;
  ...
  chartBatch.series.push({
    ...
    data: {
      ...
      data: stockPrices.map(price => ({
        x: price,
        y: isCall ? calculateCallPayoff(price, strike, premium, isLong) : calculatePutPayoff(price, strike, premium, isLong)
      })),
      className: `${isCall ? 'call' : 'put'}-${isLong ? 'long' : 'short'}-series`,
      type: "line",
      lineWidth: 1.5,
...
function updatePlotlines(strike, isCall, isLong, premium) {
  ...
  chartBatch.plotLines = [
    ...plotLines.filter(line => !line.label?.text?.includes(`${optionType} $${strike}`)),
    {
      value: strike,
...
function updateCombinedStrategy() {
  ...
  if (selectedOptions.calls.length > 0 || selectedOptions.puts.length > 0) {
    ...
    if (showCombinedOnly) {
      setTimeout(() => {
        component.chart.series.forEach(series => {
          if (series.name !== "Combined Strategy") series.setVisible(false, false);
        });
        component.chart.redraw();
      }, 20);
    }

    // Calculate and display combined breakeven points
    ...
    // Remove ALL existing breakeven lines and rebuild plotlines cleanly
    ...
    const strikePlotLines = existingPlotLines.filter((line) => {
      // Keep only strike lines, remove ALL types of breakeven lines
      const isStrikeLine =
    ...
    // Add single combined breakeven line (only if we have breakeven points)
    ...
    if (breakEvenPoints.length > 0) {
      // For multiple breakeven points, use the middle one or average
    ...
    if (strategy && strategyInfoText) {
      ...
      outlookDiv.innerHTML = `<strong>Outlook:</strong> <span class="strategy-badge ${strategyTypeClass}">${strategy.outlook.split(",")[0]}</span>`;
      ...
      strategyInfoText.append(header, description, outlookDiv, profitDiv, lossDiv);
        ...
        events: {
          cell: {
            afterSetValue: function () {
              ...
              // Restore visual state for existing selections
              ...
              const existingCall = selectedOptions.calls.find(
                call => call.strike === strikeValue && call.expiration === activeExpiration
              );
              if (existingCall) {
                longCallBtn?.classList.toggle("selected", existingCall.isLong);
                shortCallBtn?.classList.toggle("selected", !existingCall.isLong);
              }

              const existingPut = selectedOptions.puts.find(
                put => put.strike === strikeValue && put.expiration === activeExpiration
              );
              if (existingPut) {

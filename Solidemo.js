    ...
    },
    {
      ...
      document.getElementById('showCombinedOnly')?.addEventListener('change', function() {
        ...
        if (component && component.chart) {
          // Hide/show individual option series
          ...
          // Hide/show individual plotlines using the CSS classes we defined
          component.chart.xAxis[0].plotLinesAndBands.forEach(line => {
            if (line.options.className && 
                !line.options.className.includes('combined-breakeven-line')) {
              line.svgElem.css({ opacity: isChecked ? 0 : 0.4 });
              if (line.label) {
                line.label.css({ opacity: isChecked ? 0 : 1 });
              }
            }
          });
...
function showMaxOptionsWarning() {
  const strategyInfoText = document.getElementById('strategyInfoText');
  if (!strategyInfoText) return;
  
  // Store original content to restore later
  const originalContent = strategyInfoText.innerHTML;
  
  // Create warning with the CSS class we defined
  const warningHTML = `
    <div class="strategy-warning">
      <strong>Maximum Limit Reached</strong>
      <p>Maximum ${MAX_OPTIONS_IN_CHART} options can be displayed. Remove an option to add another.</p>
    </div>
    ${originalContent}
  `;
  
  strategyInfoText.innerHTML = warningHTML;
  
  // Restore original content after delay
  setTimeout(() => {
    strategyInfoText.innerHTML = originalContent;
  }, 4000);
}
